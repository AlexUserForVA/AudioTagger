...
class ProducerThread(threading.Thread):
    ...
    def run(self):
        counter = 0
        stream = self.produce(self.audioSource)
        for frame in stream:
            # counter += 1
            if self._stopevent.isSet():
                return
            # print("Produce: " + str(counter))
            self.model.putToSM(frame)
    ...
    def produce(self, infile, **kwargs):
        ...
        # kwargs['hop_size'] = kwargs.get('hop_size', 128)
        ...
        # use the input file
            ...
            hop_size = kwargs.get('hop_size', FRAME_SIZE)
